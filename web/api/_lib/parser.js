export function parseQuestionsFromText(text) {
  ...
  for (const line of lines) {
    ...
    if (qMatch && !optMatch) {
      ...
      current = { question: qMatch[2].trim(), options: [], correctAnswer: null };
    } else if (optMatch && current) {
      const letter = optMatch[1].toUpperCase();
      const text = optMatch[2].trim();
      current.options.push(`${letter}) ${text}`);
      if (isMarkedCorrect) current.correctAnswer = letter;
    } else if (current && current.options.length === 0 && line.length > 10) {
      current.question += " " + line;
    }
  }

  if (current && current.options.length >= 2) questions.push(finalize(current));
  if (questions.length === 0) return parseFallback(text);
